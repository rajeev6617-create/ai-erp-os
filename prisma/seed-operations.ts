...
const MODULES: OperationModuleSeed[] = [
  {
    ...
    description:
      "Supplier onboarding, purchase orders, three-way matching, and payment release controls.",
    ownerRole: "finance-manager",
    financeCategory: "Accounts payable",
    stages: [
      stage("requisition", "Purchase requisition", "Demand intake and budget guardrail check.", 1, "COMPLETED", 24, "AI spend classification"),
      stage("vendor_selection", "Vendor selection", "Supplier comparison, GST validation, and risk screening.", 2, "IN_PROGRESS", 36, "AI vendor risk screen"),
      stage("po_approval", "PO approval", "Approval by value threshold and department budget owner.", 3, "WAITING_APPROVAL", 12, "Policy routing"),
      stage("goods_receipt", "Goods receipt", "GRN capture and service confirmation.", 4, "NOT_STARTED", 48, "Mobile receiving"),
      stage("invoice_match", "Invoice match", "Invoice, PO, and receipt tolerance review.", 5, "BLOCKED", 24, "Three-way match"),
      stage("payment_release", "Payment release", "Treasury approval, bank file creation, and payment posting.", 6, "WAITING_APPROVAL", 8, "Payment run controls"),
    ],
    approvalFlows: [
      flow("Purchase value control", "Routes high-value purchase orders to department and finance approvers.", "PO amount above INR 5L", 1, "manager", "SEQUENTIAL", 500000),
      flow("Vendor payment control", "Requires finance approval before payment file release.", "Payment batch above INR 2L", 2, "finance-manager", "SEQUENTIAL", 200000),
      flow("CFO exception approval", "Escalates tolerance breaks and urgent payments.", "Match variance or urgent payment flag", 3, "cfo", "ANY_ONE", 1000000),
    ],
    records: [
      record("P2P-PO-1048", "Laptop refresh procurement", "Procurement package awaiting finance approval.", "po_approval", "WAITING_APPROVAL", 890000, "TechNova Systems", "manager", 12, undefined, 64, { department: "IT", budgetAvailable: true }),
      record("P2P-INV-3181", "Invoice match variance", "Supplier invoice exceeds PO tolerance by 4.8%.", "invoice_match", "EXCEPTION", 1265000, "Shakti Industrial Supplies", "finance-manager", 6, undefined, 87, { variancePercent: 4.8, tolerancePercent: 2 }),
      record("P2P-PAY-2210", "Weekly vendor payment run", "Payment batch prepared for treasury release.", "payment_release", "WAITING_APPROVAL", 2750000, "Multi-vendor batch", "finance-manager", 8, undefined, 72, { paymentCount: 18 }),
    ],
    riskAlerts: [
      risk("ops:P2P:risk:po-1048:single-bid", "P2P-PO-1048", "Single-bid supplier selection", "Only one compliant supplier bid is attached for a high-value purchase order.", "HIGH", "supplier_concentration", 86.5, { control: "competitive_bidding" }),
      risk("ops:P2P:risk:inv-3181:match-variance", "P2P-INV-3181", "Three-way match variance", "Invoice variance exceeds configured tolerance and blocks automated payment release.", "CRITICAL", "invoice_match_exception", 93.2, { tolerancePercent: 2 }),
    ],
    financeImpacts: [
    ...
    auditEvents: [
      audit("ops:P2P:audit:po-1048:threshold", "P2P-PO-1048", "approval.threshold.evaluated", "AI procurement control", "INFO", { threshold: 500000, approverRole: "manager" }),
