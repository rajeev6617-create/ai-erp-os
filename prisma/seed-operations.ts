...
const MODULES: OperationModuleSeed[] = [
  ...
  },
  {
    ...
    description:
      "Accounting close, reconciliation, financial statement, MIS, and audit review controls.",
    ownerRole: "auditor",
    financeCategory: "General ledger",
    stages: [
      stage("journal_entry", "Journal Entry", "Manual and system journal preparation with policy checks.", 1, "WAITING_APPROVAL", 12, "Journal policy check"),
      stage("ledger_posting", "Ledger Posting", "Approved journals posted to the general ledger.", 2, "IN_PROGRESS", 8, "Posting validation"),
      stage("bank_reconciliation", "Bank Reconciliation", "Bank, GST, sub-ledger, and control account matching.", 3, "BLOCKED", 24, "AI reconciliation"),
      stage("trial_balance", "Trial Balance", "Trial balance generation, variance review, and control tie-out.", 4, "IN_PROGRESS", 12, "Variance analytics"),
      stage("period_closing", "Period Closing", "Close checklist execution and controller sign-off.", 5, "WAITING_APPROVAL", 24, "Close task orchestration"),
      stage("pl_statement", "P&L Statement", "Revenue, expense, and margin statement preparation.", 6, "IN_PROGRESS", 12, "Statement generation"),
      stage("balance_sheet", "Balance Sheet", "Asset, liability, equity, and working capital snapshot.", 7, "IN_PROGRESS", 12, "Balance validation"),
      stage("mis_reporting", "MIS Reporting", "Executive MIS and board reporting pack preparation.", 8, "NOT_STARTED", 24, "Report pack automation"),
      stage("audit_review", "Audit Review", "Audit exception remediation and evidence sign-off.", 9, "BLOCKED", 36, "Evidence collection"),
    ],
    approvalFlows: [
      flow("Journal entry approval", "Requires reviewer approval for manual journals above materiality.", "Manual journal above INR 2L", 1, "finance-manager", "SEQUENTIAL", 200000),
      flow("Reconciliation exception sign-off", "Escalates unreconciled bank and ledger breaks above tolerance.", "Reconciliation break above INR 2.5L", 2, "auditor", "ANY_ONE", 250000),
      flow("Period close approval", "Controller sign-off before closing the accounting period.", "Close checklist completion", 3, "finance-manager", "SEQUENTIAL"),
      flow("Financial statement approval", "CFO approval for P&L, balance sheet, and MIS pack release.", "Financial statement pack ready", 4, "cfo", "SEQUENTIAL"),
      flow("Audit review remediation approval", "Auditor approval required before audit exceptions are closed.", "Audit exception remediation submitted", 5, "auditor", "ANY_ONE"),
    ],
    records: [
      record("R2R-JE-4410", "Accrual journal approval", "Marketing accrual journal requires finance manager approval before posting.", "journal_entry", "WAITING_APPROVAL", 620000, "General ledger", "finance-manager", 10, undefined, 76, { account: "Marketing expenses", journalType: "manual_accrual" }),
      record("R2R-LED-1180", "Ledger posting validation", "Approved accrual batch is queued for ledger posting and control account tie-out.", "ledger_posting", "OPEN", 620000, "General ledger", "finance-manager", 8, undefined, 54, { postingBatch: "GL-MAY-1180" }),
      record("R2R-REC-2305", "Bank reconciliation break", "Unmatched bank statement lines exceed reconciliation tolerance.", "bank_reconciliation", "EXCEPTION", 315000, "HDFC Bank", "auditor", 4, undefined, 90, { unmatchedLines: 14 }),
      record("R2R-TB-0526", "Trial balance variance review", "Trial balance has suspense and bank variance pending controller review.", "trial_balance", "OPEN", 315000, "Corporate accounting", "finance-manager", 14, undefined, 58, { suspenseAccountVariance: 315000 }),
      record("R2R-CLOSE-0526", "Month-end close checklist", "Close task pack is ready for controller approval.", "period_closing", "WAITING_APPROVAL", undefined, "Corporate accounting", "auditor", 22, undefined, 64, { closeDay: "D+2", checklistCompletion: 86 }),
      record("R2R-PL-0526", "P&L statement draft", "Monthly P&L statement is prepared with expense variance review pending.", "pl_statement", "OPEN", 3970000, "Corporate finance", "finance-manager", 18, undefined, 52, { revenue: 18400000, expense: 14430000 }),
      record("R2R-BS-0526", "Balance sheet snapshot", "Balance sheet snapshot requires asset and liability tie-out before release.", "balance_sheet", "OPEN", 12400000, "Corporate finance", "finance-manager", 20, undefined, 55, { assets: 12400000, liabilities: 7350000 }),
      record("R2R-MIS-0526", "Board MIS pack", "Executive MIS pack is waiting for final financial statement linkage.", "mis_reporting", "OPEN", undefined, "Board reporting", "cfo", 30, undefined, 49, { pack: "May 2026 Board MIS" }),
      record("R2R-AUD-9001", "Audit evidence exception", "Audit review found missing reconciliation evidence for unmatched bank lines.", "audit_review", "EXCEPTION", 315000, "External audit", "auditor", 28, undefined, 88, { evidenceGap: "bank_reconciliation_workpaper" }),
    ],
    riskAlerts: [
      risk("ops:R2R:risk:rec-2305:break", "R2R-REC-2305", "Reconciliation anomaly", "Bank reconciliation exceptions exceed materiality for the period.", "CRITICAL", "reconciliation_anomaly", 91.6, { materiality: 250000 }),
      risk("ops:R2R:risk:je-4410:evidence", "R2R-JE-4410", "Unusual journal entry", "Manual accrual journal has elevated amount, late-cycle timing, and incomplete supporting evidence.", "HIGH", "unusual_journal_entry", 86.8, { evidenceRequired: true }),
      risk("ops:R2R:risk:close-0526:delay", "R2R-CLOSE-0526", "Closing delay prediction", "Close checklist dependency on reconciliation remediation may delay month-end sign-off.", "HIGH", "closing_delay_prediction", 84.4, { predictedDelayHours: 18 }),
      risk("ops:R2R:risk:pl-0526:expense-variance", "R2R-PL-0526", "Expense variance alert", "Operating expense variance is above policy threshold versus forecast.", "MEDIUM", "expense_variance_alert", 78.1, { variancePercent: 12.4 }),
      risk("ops:R2R:risk:aud-9001:audit-risk", "R2R-AUD-9001", "Audit risk signal", "Audit review evidence gap is linked to a material reconciliation break.", "HIGH", "audit_risk_signal", 88.9, { auditArea: "cash_and_bank" }),
    ],
    financeImpacts: [
      impact("ops:R2R:impact:je-4410", "R2R-JE-4410", "journal_accrual", "Accrual awaiting approval", 620000, "OUTFLOW", "May 2026"),
      impact("ops:R2R:impact:rec-2305", "R2R-REC-2305", "reconciliation_variance", "Unreconciled cash movement", 315000, "NEUTRAL", "May 2026"),
      impact("ops:R2R:impact:close-0526", "R2R-CLOSE-0526", "close_readiness", "Close checklist exposure", 0, "NEUTRAL", "May 2026"),
      impact("ops:R2R:impact:tb-0526", "R2R-TB-0526", "trial_balance_variance", "Trial balance variance under review", 315000, "NEUTRAL", "May 2026"),
      impact("ops:R2R:impact:pl-revenue", "R2R-PL-0526", "pl_revenue", "P&L revenue recognized", 18400000, "INFLOW", "May 2026"),
      impact("ops:R2R:impact:pl-expense", "R2R-PL-0526", "pl_expense", "P&L expense base", 14430000, "OUTFLOW", "May 2026"),
      impact("ops:R2R:impact:bs-assets", "R2R-BS-0526", "balance_sheet_asset", "Balance sheet assets", 12400000, "NEUTRAL", "May 2026"),
      impact("ops:R2R:impact:bs-liabilities", "R2R-BS-0526", "balance_sheet_liability", "Balance sheet liabilities", 7350000, "NEUTRAL", "May 2026"),
      impact("ops:R2R:impact:audit-exception", "R2R-AUD-9001", "audit_exception_exposure", "Audit exception exposure", 315000, "NEUTRAL", "May 2026"),
    ],
    auditEvents: [
      audit("ops:R2R:audit:je-4410:evidence", "R2R-JE-4410", "journal.entry.approval.requested", "GL reviewer", "INFO", { evidenceType: "vendor_quote" }),
      audit("ops:R2R:audit:led-1180:posting", "R2R-LED-1180", "ledger.posting.queued", "GL automation", "INFO", { postingBatch: "GL-MAY-1180" }),
      audit("ops:R2R:audit:rec-2305:block", "R2R-REC-2305", "bank.reconciliation.blocked", "AI reconciliation agent", "ERROR", { unmatchedLines: 14 }),
      audit("ops:R2R:audit:tb-0526:generated", "R2R-TB-0526", "trial.balance.generated", "Controller", "INFO", { varianceAmount: 315000 }),
      audit("ops:R2R:audit:close-0526:pending", "R2R-CLOSE-0526", "period.close.checklist.pending", "Close orchestration", "WARNING", { checklistCompletion: 86 }),
      audit("ops:R2R:audit:pl-0526:prepared", "R2R-PL-0526", "pl.statement.prepared", "Finance analyst", "INFO", { period: "May 2026" }),
      audit("ops:R2R:audit:bs-0526:snapshot", "R2R-BS-0526", "balance.sheet.snapshot.generated", "Finance analyst", "INFO", { period: "May 2026" }),
      audit("ops:R2R:audit:mis-0526:pack", "R2R-MIS-0526", "mis.reporting.pack.prepared", "Board reporting", "INFO", { pack: "May 2026 Board MIS" }),
      audit("ops:R2R:audit:aud-9001:exception", "R2R-AUD-9001", "audit.review.exception.logged", "Audit reviewer", "CRITICAL", { evidenceGap: "bank_reconciliation_workpaper" }),
    ],
  },
  {
